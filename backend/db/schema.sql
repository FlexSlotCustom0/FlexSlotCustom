...
BEGIN
    -- Merge logic: current ui_config || p_default_config
    -- Note: || operator in JSONB merges keys. If a key exists in both, the right one wins.
    -- To preserve "brand identity" like logo, we ensure the existing config's specific fields are kept if they exist.
    
    UPDATE public.clinics
    SET 
        template_id = p_template_id,
        ui_config = p_default_config || ui_config -- Existing ui_config overrides defaults (right side wins)
